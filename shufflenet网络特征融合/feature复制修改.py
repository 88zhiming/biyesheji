...
def hsv(img1):
    # img = cv2.imread(img1)  # 读一张彩色图片
    # if img is None:
    #     return

    hsv = cv2.cvtColor(img1, cv2.COLOR_BGR2HSV)  # RGB空间转换为HSV空间
    h, s, v = cv2.split(hsv)
...
#/////////////////////////////////////////////////////////////////////
    ...
    hsv_feature =np.squeeze(hsv_feature)
    # print(hsv_feature.shape)
    # hsv_feature = np.expand_dims(hsv_feature, 0)
    # print(hsv_feature.shape)
    # hsv_feature=np.squeeze(hsv_feature)
    # print(hsv_feature.shape)
    # 计算图像对比度信息
    ...
    # lbp_feature = np.squeeze(lbp_feature)
    # lbp_feature = np.expand_dims(lbp_feature, 0)
    # lbp_feature =  np.expand_dims(lbp_feature, 0)
    #计算白色占比
    white_feature=white(img2)
    # white_feature = np.array(white_feature)
    # white_feature = np.expand_dims(white_feature, 0)
    # white_feature = np.squeeze(white_feature)
    #灰度共生矩阵
    huidugongshengjuzheng_featuer=glcm(img2)
    # huidugongshengjuzheng_featuer = np.squeeze(huidugongshengjuzheng_featuer)
# /////////////////////////////////////////////////////////////////////
    # 平均梯度信息
    gd_img = Gradient(image)
    fmgd = Histogram_cal(gd_img * 255)
    fmgd = np.squeeze(fmgd)
#hsv_feature,lbp_feature,white_feature,huidugongshengjuzheng_featuer,
